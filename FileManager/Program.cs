...
using FL = FileManager.BusinessLayer.Helpers.FileFormats;
...
namespace FileManager
{
    class Program
    {
        ...
        {
            //var testEpisode = Episode.FindEpisode("Test Episode");

            foreach(var testEpisode in Episode.GetEpisodes())
            {
                Console.WriteLine(testEpisode.Name);
                Console.WriteLine(testEpisode.Path);
            }
        ...
        private static string GetUserPath()
        {
            ...
            return @"\\MACBOOKPRO-615C\aletinski\Downloads";
        }
        ...
        private static void MoveContentOfDirectory(IEnumerable<string> directories, string newestSeasonDirectory)
        {
            foreach (var directory in directories)
            {
                ...
                {
                    if (!Enum.IsDefined(typeof(FL.FileFormatTypes), file.Split('.').Last()))
                        continue;
